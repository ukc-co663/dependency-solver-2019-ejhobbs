...
int repo_getPackageFromIndex(const repository *repo, const relation *r,
                             int idx) {
  int max = repo->size;
  ...
  if (compareNames(repo, r->name, idx) != 0)
    return -1; /* went past where it should have been */

  /* Increase until we find the first pkg to satisfy the constraint */
  while (idx < max && compareNames(repo, r->name, idx) == 0 &&
         satisfiesConstraint(repo, r, idx) != 1) {
    idx++;
  }
...
void package_free(package *p) {
  ...
  if (p->cConflicts > 0 && p->conflicts != NULL)
    relation_freeAll(p->cConflicts, p->conflicts);
